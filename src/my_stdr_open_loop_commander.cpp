...
int main(int argc, char **argv) {
    ...
    while(timer<time_3_sec) { //move for 3 seconds - 3 meters to the right
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
    ...
    twist_cmd.angular.z=yaw_rate; //and start spinning in place - spin to face up
    ...
    timer=0.0; //reset the timer
    while(timer<time_3_sec) { // move for 3 seconds - 3 meters up
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
    ...
    twist_cmd.linear.x=speed; //and move forward again
    timer=0.0; //reset the timer
    while(timer<3.5) { //move for 3.5 seconds (3 meters left)
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
    ...
    timer=0.0; //reset the timer
    while(timer<time_3_sec) { //rotate to face upwards (clockwise)
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
    ...
    twist_cmd.linear.x=speed; //and move forward again
    timer=0.0; //reset the timer
    while(timer<7.0) { //move for 7 seconds (up 7 meters)
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
    ...
    twist_cmd.angular.z=yaw_rate; //and start spinning in place
    timer=0.0; //reset the timer
    while(timer<time_3_sec) { //rotate counterclockwise to face left
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
    ...
    twist_cmd.linear.x=speed; //and move forward again
    timer=0.0; //reset the timer
    while(timer<3) { //move for 3 seconds (left 3 meters) 
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
